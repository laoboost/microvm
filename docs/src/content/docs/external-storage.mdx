...
The daemon also pins `--maximum-throughput-gbps` by default (value `10`): without it
mount-s3 probes IMDS for the instance throughput, which burns ~4s per mount and
fails on bare-metal runners without IMDS. An explicit `maximum_throughput_gbps`
option (or the same flag in legacy `extra_args`) overrides the pin.
